...
One chart ships with open-source BEAR:

- **BEAR Position Grid** (the signature 2x2 visual, generated by `tools/position-grid.py`)

The operational version includes additional chart types (timeline, search-lead-gap, convergence map, cost pressure bars) for client presentations. The markdown tables in the open-source deliverable tell the same story.
...
| Weekly pulse | Not included | ~5 min automated pulse with API data |
